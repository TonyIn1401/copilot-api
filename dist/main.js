...
const githubApiBaseUrl = (state$1) => state$1.githubUrl ? `${state$1.githubUrl.replace(/\/$/, "")}/api/v3` : "https://api.github.com";
...
const githubBaseUrl = (state$1) => state$1.githubUrl ? state$1.githubUrl.replace(/\/$/, "") : "https://github.com";
...
const getCopilotToken = async () => {
	const response = await fetch(`${githubApiBaseUrl(state)}/copilot_internal/v2/token`, { headers: githubHeaders(state) });
...
async function getGitHubUser() {
	const response = await fetch(`${githubApiBaseUrl(state)}/user`, { headers: {
...
const auth = defineCommand({
	...
	args: {
		...
		"github-url": {
			type: "string",
			description: "GitHub Enterprise URL (e.g., https://github.example.com). Can also be set via GITHUB_URL env var"
		}
	},
	run({ args }) {
		return runAuth({
